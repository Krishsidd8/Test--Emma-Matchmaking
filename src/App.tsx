...
const REVEAL_DATE = new Date(REVEAL_DATE_ISO); // create once, avoid re-creation each render
...
const Matchmaking: React.FC = () => {
  ...
  // Load previously-saved user (prevents re-submission)
  const stored = typeof window !== 'undefined' ? localStorage.getItem('emma_user') : null;
  const parsedUser: User | null = stored ? JSON.parse(stored) : null;

  const [user, setUser] = useState<User | null>(parsedUser);
  const [step, setStep] = useState<'signup' | 'matchType' | 'questions' | 'waiting'>(parsedUser ? 'waiting' : 'signup');

  const [form, setForm] = useState({ name: '', email: '', grade: '' });
  const [matchType, setMatchType] = useState<string>('');
  const [answers, setAnswers] = useState<Record<string, string>>({});

  // Time-left countdown (ms)
  const [timeLeftMs, setTimeLeftMs] = useState<number>(Math.max(0, REVEAL_DATE.getTime() - Date.now()));

  useEffect(() => {
    if (step !== 'waiting') return; // only run countdown while in waiting room
  ...
  // When reveal passes and user is waiting, navigate to reveal route
  useEffect(() => {
  ...
  const handleSignup = () => {
    ...
    // If user already submitted with this email, jump to waiting room
    const storedAll = localStorage.getItem('emma_user');
    if (storedAll) {
      const u = JSON.parse(storedAll) as User;
  ...
  const handleSelectMatchType = (type: string) => {
    setMatchType(type);
    setStep('questions');
  };
  ...
  const handleSubmit = () => {
    ...
    const u: User = { ...form, matchType, answers, submittedAt: new Date().toISOString() };
...
// ---- Reveal component (top-level route will render this) ----
const Reveal: React.FC<{ user: User | null }> = ({ user }) => {
  if (!user) {
    return (
